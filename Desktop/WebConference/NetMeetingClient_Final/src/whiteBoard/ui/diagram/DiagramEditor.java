...
import whiteBoard.SetLable;
import whiteBoard.draw.Connection;
import whiteBoard.draw.DiagramElement;
import whiteBoard.draw.DiagramOperations;
import whiteBoard.draw.DrawingContext;
import whiteBoard.draw.DrawingContextImpl;
import whiteBoard.draw.Label;
import whiteBoard.draw.MultiLineLabel;
import whiteBoard.draw.Node;
import whiteBoard.draw.NodeChangeListener;
import whiteBoard.draw.RectilinearConnection;
import whiteBoard.draw.Scaling;
import whiteBoard.draw.SimpleConnection;
import whiteBoard.draw.DrawingContext.FontType;
import whiteBoard.model.ElementType;
import whiteBoard.model.Relation;
import whiteBoard.model.RelationEndType;
import whiteBoard.model.RelationType;
import whiteBoard.model.UmlClass;
import whiteBoard.ui.diagram.commands.ConvertConnectionTypeCommand;
import whiteBoard.ui.diagram.commands.DeleteElementCommand;
import whiteBoard.ui.diagram.commands.DiagramEditorNotification;
import whiteBoard.ui.diagram.commands.EditConnectionPointsCommand;
import whiteBoard.ui.diagram.commands.MoveElementCommand;
import whiteBoard.ui.diagram.commands.ResetConnectionPointsCommand;
import whiteBoard.ui.diagram.commands.ResizeElementCommand;
import whiteBoard.ui.diagram.commands.SetConnectionNavigabilityCommand;
import whiteBoard.ui.diagram.commands.SetLabelTextCommand;
...
/**
 * This class represents the diagram editor. It mainly acts as the component to
 * draw the diagram and to handle the events from the input system. The actual
 * drawing is handled by the Diagram class and its sub elements.
 * 
 * @author Wei-ju Wu
 * @version 1.0
 */
public class DiagramEditor extends JComponent implements ActionListener,
		MouseListener, MouseMotionListener, DiagramEditorNotification,
		DiagramOperations, NodeChangeListener, Serializable {
	private boolean owerchange = false;
	...
	public transient SelectionHandler selectionHandler;
	...
	private TextEditor currentEditor = null;
	 ...
	 *            the diagram
	 */
	public DiagramEditor(JPanel aboard, StructureDiagram aDiagram) {
		board = aboard;
		diagram = aDiagram;
	...
	private void setToDiagramSize() {
		setPreferredSize(new Dimension((int) (diagram.getSize().getWidth()
				+ MARGIN_RIGHT + MARGIN_LEFT), (int) (diagram.getSize()
	...
	private void installHandlers() {
		...
		getInputMap(WHEN_IN_FOCUSED_WINDOW).put(
				KeyStroke.getKeyStroke("DELETE"), "deleteSelection");
	 ...
	 * @param bounds
	 *            the bounding rectangle to repaint
	 * @param toScreen
	 *            true if rendered to screen, false otherwise otherwise
	 */
	 ...
	 */
	public void actionPerformed(ActionEvent e) {
		this.owerchange = true;
		sendactionPerformed(e);
		stopEditing();
		this.owerchange = false;
	}

	private void sendactionPerformed(ActionEvent e) {
		try {
			Client.writeObject(new ActionEventNet(e, ActionEnum.DACTIONEVENT));
		} catch (Exception x) {
			System.out.println("Send sendmouseReleased error");
		}
	}
	...
	private void sendLableText(String text) {
		try {
			Client.writeObject(new SetLable(null, text));
		} catch (Exception x) {
			System.out.println("Send sendLableText error");
		}
	}

	public void setElementLable(String text) {
		Label label = currentEditor.getLabel();
		SetLabelTextCommand command = new SetLabelTextCommand(label, text);
		execute(command);
		currentEditor.hideEditor();
	...
	private void sendmousePressed(MouseEvent e) {
		try {
			Client.writeObject(new ActionEventNet(e, ActionEnum.DMOUSEPRESS));
		} catch (Exception x) {
			System.out.println("Send sendmousePressed error");
		}
	...
	private void sendmouseReleased(MouseEvent e) {
		try {
			Client.writeObject(new ActionEventNet(e, ActionEnum.DMOUSERELEASED));
		} catch (Exception x) {
			System.out.println("Send sendmouseReleased error");
		}
	}
	...
	private void sendmouseClicked(MouseEvent e) {
		try {
			Client.writeObject(new ActionEventNet(e, ActionEnum.DMOUSECLICKED));
		} catch (Exception x) {
			System.out.println("Send sendmouseClicked error");
		}
	}
	...
	private void sendmouseMoved(MouseEvent e) {
		// TODO Auto-generated method stub
		try {
			Client.writeObject(new ActionEventNet(e, ActionEnum.DMOUSEMOVED));
		} catch (Exception x) {
			System.out.println("Send sendmouseMoved error");
		}
	}
	...
	private void sendmouseDragged(MouseEvent e) {
		try {
			Client.writeObject(new ActionEventNet(e, ActionEnum.DMOUSEDRAGGED));
		} catch (Exception x) {
			System.out.println("Send sendmouseDragged error");
		}
	}

	/**
	 * Notifies the coordinate listeners. Precondition: Mouse coordinates have
	 ...
	 * 
	 * @param elementType
	 *            the ElementType that indicates what to create
	 */
	 ...
	 * 
	 * @param relationType
	 *            the RelationType to create
	 */
	public void setCreateConnectionMode(RelationType relationType) {
		lineHandler.setRelationType(relationType, getDiagram()
				.getElementFactory().getConnectMethod(relationType));
	 ...
	 * 
	 * @param flag
	 *            true for visible grid, false otherwise
	 */
	 ...
	 * 
	 * @param flag
	 *            true if snapping should be supported, false otherwise
	 */
	...
	public void resetConnectionPoints() {
		...
		if (elem instanceof Connection) {
			sendResetConnectionPointsCommandNet(elem.hashCode());
			execute(new ResetConnectionPointsCommand(this, (Connection) elem));
		}
	}

	private void sendResetConnectionPointsCommandNet(int hashCode) {
	...
	public void bringToFront() {
		if (selectionHandler.getSelectedElements().size() > 0) {
			diagram.bringChildToFront(selectionHandler.getSelectedElements()
					.get(0));
			redraw();
		}
	}
	...
	public void rectilinearToDirect() {
		if (getSelectedElements().size() > 0
			...
			// we can only tell the selection handler to forget about the
			// selection
			selectionHandler.deselectAll();
		}
	}

	private void sendMessage(UmlConnection conn) {
		Client.writeObject(conn);
		Client.writeObject(conn);
	...
	public void setNavigability(RelationEndType endType) {
		if (getSelectedElements().size() > 0
			...
			if (endType == RelationEndType.SOURCE) {
				execute(new SetConnectionNavigabilityCommand(this, conn,
						endType, !relation.isNavigableToElement1()));
			}
			if (endType == RelationEndType.TARGET) {
				execute(new SetConnectionNavigabilityCommand(this, conn,
						endType, !relation.isNavigableToElement2()));
			}
		}
	}
	...
	public void editProperties(DiagramElement element) {
		...
		// EditClassDialog dialog = new EditClassDialog(board, classElement,
		// true);
		...
		// EditAssociationDialog dialog = new EditAssociationDialog(board,
		// association, true);
		// dialog.setLocationRelativeTo(board);
		// dialog.setVisible(true);
		// redraw();
	...
	public void resizeElement(Node element, Point2D newpos, Dimension2D size) {
		ResizeElementCommand cmd = new ResizeElementCommand(this, element,
				newpos, size);
	...
	public void editLabel(Label label) {
		if (label != null) {
			if (label instanceof MultiLineLabel) {
				multilineEditor.setFont(drawingContext
						.getFont(FontType.DEFAULT));
	...
	public void addConnectionCommand(DiagramElement elem, UmlNode source) {
		lineHandler.mouseReleasedNet(elem, source);
	}

	// net method
	public void mousePressedNet(MouseEvent awtevent) {
		if (!stopEditing()) {
			editorMode.mousePressed(convertMouseEvent(awtevent));
		}
	}

	public void mouseClickedNet(MouseEvent awtevent) {
		if (!stopEditing()) {

			editorMode.mouseClicked(convertMouseEvent(awtevent));
		}
	}

	public void mouseReleasedNet(MouseEvent awtevent) {
		if (!stopEditing()) {

			editorMode.mouseReleased(convertMouseEvent(awtevent));
		}
	}

	public void mouseEnteredNet(MouseEvent awtevent) {
		// TODO Auto-generated method stub

	}

	public void mouseExitedNet(MouseEvent awtevent) {
		// TODO Auto-generated method stub

	}

	public void mouseMovedNet(MouseEvent awtevent) {
		EditorMouseEvent evt = convertMouseEvent(awtevent);
		editorMode.mouseMoved(evt);
		notifyCoordinateListeners();
	}

	public void mouseDraggedNet(MouseEvent awtevent) {
		EditorMouseEvent evt = convertMouseEvent(awtevent);
		editorMode.mouseDragged(evt);
		notifyCoordinateListeners();
	}

	public void actionPerformedNet(ActionEvent awtevent) {
		stopEditing();
	}
