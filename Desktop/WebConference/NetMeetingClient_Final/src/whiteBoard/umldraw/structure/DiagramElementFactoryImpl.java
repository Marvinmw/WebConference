...
import whiteBoard.model.UmlComponent;
import whiteBoard.model.UmlPackage;
import whiteBoard.model.UmlRelation;
...
public class DiagramElementFactoryImpl implements DiagramElementFactory{
  ...
  private void setupElementPrototypeMap() {
   ...
   //Add actor
    UmlComponent actor = (UmlComponent) UmlComponent.getPrototype().clone();
    actor.setName("Actor");
    Actor actorElem = (Actor)
      Actor.getPrototype().clone();
    actorElem.setModelElement(actor);
    actorElem.addNodeChangeListener(diagram);
    elementPrototypes.put(ElementType.ACTOR, actorElem);
 //Add SystemBound
    UmlComponent systemBound = (UmlComponent) UmlComponent.getPrototype().clone();
    systemBound.setName("System");
    SystemBound systemBoundElem = (SystemBound)
     SystemBound.getPrototype().clone();
    systemBoundElem.setModelElement(systemBound);
    systemBoundElem.addNodeChangeListener(diagram);
    elementPrototypes.put(ElementType.SYSTEMBOUND, systemBoundElem);
  //Add Rect
    UmlComponent rect = (UmlComponent) UmlComponent.getPrototype().clone();
    rect.setName("Rect");
    Rect rectElem = (Rect)
     Rect.getPrototype().clone();
    rectElem.setModelElement(rect);
    rectElem.addNodeChangeListener(diagram);
    elementPrototypes.put(ElementType.RECT, rectElem);
  //Add Oval
    UmlComponent oval = (UmlComponent) UmlComponent.getPrototype().clone();
    oval.setName("Oval");
    Oval ovalElem = (Oval)
     Oval.getPrototype().clone();
    ovalElem.setModelElement(oval);
    ovalElem.addNodeChangeListener(diagram);
    elementPrototypes.put(ElementType.OVAL, ovalElem);
