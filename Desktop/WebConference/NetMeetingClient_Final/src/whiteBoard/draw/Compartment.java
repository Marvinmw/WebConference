...
implements CompositeNode ,Serializable{
...
public void drawOval(DrawingContext drawingContext) {
	if (!isValid()) {
	      recalculateSize(drawingContext);
	    }
	    drawingContext.drawOval(getAbsoluteX1(), getAbsoluteY1(),
	      getSize().getWidth(), getSize().getHeight(),Color.RED);
	    for (Label label : labels) {
	      // for now, we always center the labels
	      if (alignment == Alignment.CENTERED) label.centerHorizontally();
	      else if (alignment == Alignment.LEFT) {
