...
public class StaticStructureEditorToolbarManager implements ActionListener,
	...
	public StaticStructureEditorToolbarManager() {
		...
		toolbar.add(createToggleButton(buttongroup, "systembound"));
		...
		toolbar.add(createToggleButton(buttongroup, "rect"));
		toolbar.add(createToggleButton(buttongroup, "oval"));
		doClick("SELECT_MODE");
	}
	 ...
	 * 
	 * @param l
	 *            the AppCommandListener to remove
	 */
	...
	private void sendActionEvent(ActionEvent e) {
		try {
			Client.writeObject(new ActionEventNet(e, ActionEnum.SACTIONEVENT));
		} catch (Exception x) {
			System.out.println("Send ActionEvent error");
		}
	}
	 ...
	 * 
	 * @param actionCommand
	 *            the action command string that is tied to the button
	 * @param flag
	 ...
	 * 
	 * @param aButtonGroup
	 *            an optional ButtonGroup to add to
	 * @param name
	...
	private JToggleButton createToggleButton(ButtonGroup aButtonGroup,
		...
		JToggleButton button = new JToggleButton(IconLoader.getInstance()
				.getIcon(getResourceString(prefix + ".icon")));
	...
	public void actionPerformedNet(ActionEvent awtevent) {
		for (AppCommandListener l : listeners) {
			l.handleCommand(awtevent.getActionCommand());
		}
	}
}
